...
from keras.layers import Input, Dense, Reshape, Lambda, LSTM, CuDNNLSTM, Flatten, Concatenate, TimeDistributed, UpSampling1D
...
def build_model(input_shape, rnn_units):
    ...
    #_, encoder_h, encoder_c = CuDNNLSTM(rnn_units, return_state=True)(x, initial_state=[state_h, state_c])
    
    x = UpSampling1D(size=96//input_sequence_length)(x)
    x = LSTM(rnn_units, return_sequences=True, unroll=True)(x, initial_state=[encoder_h, encoder_c])
    #x = CuDNNLSTM(rnn_units, return_sequences=True)(x, initial_state=[encoder_h, encoder_c])
