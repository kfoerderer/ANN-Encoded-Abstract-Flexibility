...
from keras.layers import Input, Dense, Reshape, Lambda, Conv1D, Flatten, Concatenate, Permute, AveragePooling1D
...
def build_model(input_shape):
    ...
    extended = Reshape((input_shape[0],1))(input)

    split_state = Lambda(lambda x: x[:, :5, 0])(extended)
    split_sequence = Lambda(lambda x: x[:, 5:, :])(extended)

    x = Conv1D(32, 5)(split_sequence)
    x = Flatten()(x)
    x = Concatenate()([x, split_state])
